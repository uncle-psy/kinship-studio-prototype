...
const SAMPLE_ALIGNMENTS: Alignment[] = [
  {
    id: "align-1",
    name: "TerraAI Climate Prediction Pipeline",
    goal: "Ingest satellite imagery, run crop yield predictions, generate farmer advisories, and score recommendations against soil health and economic impact criteria",
    status: "active",
    steps: [
      { id: "s1", type: "action", label: "Ingest satellite data", description: "Pull latest satellite imagery and weather data for target farming regions" },
      { id: "s2", type: "tool", label: "Run ML prediction model", description: "Execute climate prediction model against crop yield dataset" },
      { id: "s3", type: "checkpoint", label: "Confidence check", description: "Validate prediction confidence > 0.85 before proceeding" },
      { id: "s4", type: "parallel", label: "Generate advisories", description: "Produce region-specific farmer advisories in parallel for each sub-region" },
      { id: "s5", type: "reflection", label: "Impact scoring", description: "Score advisories against dual criteria: economic value and soil health benefit" },
      { id: "s6", type: "decision", label: "Escalation gate", description: "If crop loss risk > 30%, escalate to Dr. Okafor for emergency advisory" },
      { id: "s7", type: "human-approval", label: "Founder review", description: "High-impact advisories require human approval before distribution" },
      { id: "s8", type: "action", label: "Distribute to farmers", description: "Push approved advisories to farming communities via mobile network" },
    ],
    databases: [
      { id: "db1", label: "Pinecone (climate embeddings)", type: "vector", connected: true },
      { id: "db2", label: "PostgreSQL (crop yields + outcomes)", type: "relational", connected: true },
    ],
    model: "claude-opus-4-6",
    systemPrompt: "You are the TerraAI orchestration agent. Your goal is to transform raw satellite and climate data into actionable advisories that protect smallholder farmers from crop losses while promoting regenerative agriculture practices.",
    checkpointing: true,
    scoringEnabled: true,
  ...
  {
    id: "align-2",
    name: "EquiLend Loan Processing Workflow",
    goal: "Evaluate loan applications, assess community impact, enforce APR caps, and route approvals through the governance framework",
    status: "ready",
    steps: [
      { id: "s2-1", type: "action", label: "Receive application", description: "Intake loan application with borrower profile and business plan" },
      { id: "s2-2", type: "tool", label: "Credit assessment", description: "Run reputation-based social collateral scoring (no asset seizure)" },
      { id: "s2-3", type: "checkpoint", label: "APR cap enforcement", description: "Verify loan terms comply with 3.5% APR cap — on-chain enforcement" },
      { id: "s2-4", type: "decision", label: "Risk threshold", description: "If risk score > acceptable threshold, route to RISK-Guardian for review" },
      { id: "s2-5", type: "reflection", label: "Community impact review", description: "Score application against community development fund criteria" },
      { id: "s2-6", type: "human-approval", label: "Final approval", description: "Loans above $5K require human governance approval" },
    ],
    databases: [
      { id: "db3", label: "Pinecone (borrower profiles)", type: "vector", connected: true },
      { id: "db4", label: "PostgreSQL (lending ledger)", type: "relational", connected: true },
    ],
    model: "claude-opus-4-6",
    systemPrompt: "You are the EquiLend loan processing orchestrator. Evaluate applications with dual focus: financial sustainability and community empowerment. Never approve terms that violate the 3.5% APR cap.",
    checkpointing: true,
    scoringEnabled: true,
...
const SAMPLE_GATHERINGS: Gathering[] = [
  {
    id: "gather-terra-expansion",
    name: "TerraAI SE Asia Expansion Proposal",
    purpose: "Multi-agent governance debate on expanding climate prediction coverage to Vietnam, Cambodia, and Myanmar — 200,000 TERRA tokens at stake",
    status: "active",
    participants: [
      { id: "p1", name: "TERRA-Presence", role: "host", type: "agent" },
      { id: "p2", name: "VALUE-Analyst", role: "participant", type: "agent" },
      { id: "p3", name: "RISK-Guardian", role: "participant", type: "agent" },
      { id: "p4", name: "Dr. Amara Okafor", role: "participant", type: "human" },
      { id: "p5", name: "Community Voters", role: "observer", type: "human" },
    ],
    vibeIds: ["kinship-constitution", "growth-space"],
    approvalRules: [
      { id: "ar1", trigger: "Token allocation > 100,000", threshold: "Always", approver: "Community vote (51% quorum)" },
      { id: "ar2", trigger: "New region deployment", threshold: "Always", approver: "Project founder + Platform Agent" },
    ],
    createdAt: "2026-03-01",
  },
  {
    id: "gather-equi-pool",
    name: "EquiLend Pool Expansion Review",
    purpose: "Governance review of raising lending pool cap from $5M to $10M with 2,300 pending applications — balancing growth with risk management",
    status: "active",
    participants: [
      { id: "p6", name: "EQUI-Presence", role: "host", type: "agent" },
      { id: "p7", name: "RISK-Guardian", role: "participant", type: "agent" },
      { id: "p8", name: "Maya Rodriguez", role: "participant", type: "human" },
      { id: "p9", name: "CIVIC-Presence", role: "observer", type: "agent" },
    ],
    vibeIds: ["kinship-constitution"],
    approvalRules: [
      { id: "ar3", trigger: "Lending pool increase > 50%", threshold: "Always", approver: "Community vote (60% quorum)" },
      { id: "ar4", trigger: "Risk score exceeds platform threshold", threshold: "Risk > 0.7", approver: "Platform Agent + Founder" },
    ],
    createdAt: "2026-03-05",
  },
  {
    id: "gather-helix-appalachia",
    name: "Helix Health Appalachian Deployment",
    purpose: "Proposal review for deploying 12 telehealth nodes across rural Appalachian clinics — addressing 2x maternal mortality rate with $4.2M in projected savings",
    status: "draft",
    participants: [
      { id: "p10", name: "HELIX-Presence", role: "host", type: "agent" },
      { id: "p11", name: "EQUITY-Advocate", role: "participant", type: "agent" },
      { id: "p12", name: "Dr. James Whitfield", role: "participant", type: "human" },
    ],
    vibeIds: ["growth-space", "kinship-constitution"],
    approvalRules: [
      { id: "ar5", trigger: "Clinical deployment", threshold: "Always", approver: "Medical advisory board" },
      { id: "ar6", trigger: "Budget allocation > $1M", threshold: "Always", approver: "Community vote + founder" },
    ],
